 ...
 * @param {string} [deviceName] - device name to capture screenshot from
 * @param {string} [screenshotPath] - path to save screenshot to, defaults
 *  to ares-screenshot-<timestamp>.png in the current working directory
 ...
 */
export async function captureScreenshot(bin, deviceName, screenshotPath) {
  log.info(`Capturing screenshot from device`);
  if (!screenshotPath) {
    screenshotPath = path.resolve(process.cwd(), `ares-screenshot-${Date.now()}.png`);
  }
  await runDeviceCmd(LUNA_SEND, deviceName, ['-n', '1', '-f', 'luna://com.webos.surfacemanager/captureCompositorOutput', `{"output":"${screenshotPath}","format":"PNG"}`]);
  log.info(`Screenshot saved to ${screenshotPath}`);
  if (!await fs.exists(screenshotPath)) {
    throw new Error(`Screenshot file does not exist at ${screenshotPath}`);
  }
  // Read the screenshot png file and encode it to base64
  // This is not strictly necessary, but it allows us to return the screenshot data
...
/**COPY src/appium-lg-webos-driver /appium-lg-webos-driver
...
export async function closeApp(appId, deviceName) {
  ...
  await runDeviceCmd(ARES_LAUNCH, deviceName, ['--close', appId]);
}
