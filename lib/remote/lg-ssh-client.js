...
export class LGSshClient {
  ...
  /**
   * Close the current app. close current app on using: luna://com.webos.service.applicationmanager/closeByAppId
   * @param {string} appId
    
   */
  async closeApp(appId) {
    log.info(`Closing app '${appId}'`);
    return this.lunaSend('luna://com.webos.service.applicationmanager', 'closeByAppId', {id: appId}); 
  }
}
